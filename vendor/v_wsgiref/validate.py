...
def validator(application):
    ...
    def lint_app(*args, **kw):
        ...
        assert_(iterator is not None and iterator, 'The application must return an iterator, if only an empty list')
...
def check_environ(environ):
    ...
    for key in environ.keys():
        ...
        assert_(
            ...
            'Environmental variable %s is not a string: %r (value: %r)' % (key, type(environ[key]), environ[key]),
        )
    ...
    assert_(
        environ.get('SCRIPT_NAME') != '/', "SCRIPT_NAME cannot be '/'; it should instead be '', and PATH_INFO should be '/'"
    )
