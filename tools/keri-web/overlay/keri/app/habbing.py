...
class Habery:
    ...
    async def aclose(self, clear=False):
        """Browser adaptation: async close for browser-backed stores that need awaited flushes."""
        if self.ks:
            if hasattr(self.ks, "aclose"):
                await self.ks.aclose(clear=self.ks.temp or clear)
            else:
                self.ks.close(clear=self.ks.temp or clear)

        if self.db:
            if hasattr(self.db, "aclose"):
                await self.db.aclose(clear=self.db.temp or clear)
            else:
                self.db.close(clear=self.db.temp or clear)
