...
from ..db import (Suber,
                  CryptSignerSuber, CesrSuber,
                  CatCesrIoSetSuber, Komer)
from ..help import nowIso8601

try:
    from ..db import openLMDB, LMDBer
except ImportError:  # Browser (Pyodide) has no lmdb; Keeper is unused there (WebKeeper is used instead).
    openLMDB = None
    LMDBer = None
...
def openKS(name="test", **kwa):
    ...
    """
    if openLMDB is None:
        raise RuntimeError("LMDB is not available in this environment (browser); use WebKeeper.")
    return openLMDB(cls=Keeper, name=name, **kwa)


class Keeper(LMDBer if LMDBer is not None else object):
    """
