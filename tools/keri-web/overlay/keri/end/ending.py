...
try:
    import falcon
except ImportError:  # Browser (Pyodide) has no falcon; only needed for HTTP server endpoints.
    falcon = None
from hio import base
try:
    from hio.core import http, wiring  # noqa: F401 (used only by server setup, never in browser)
except ImportError:  # Browser (Pyodide): hio.core.http/tcp need ssl which is unavailable.
    http = None
    wiring = None
